...
# Import HMS modules
from src.models.resnet1d_gru import ResNet1D_GRU
...
class EnhancedNovitaTrainingPipeline:
    ...
    def __init__(self, config_path: str, resume_from: Optional[str] = None):
        ...
        # Setup environment first to get device
        self._setup_environment()
        
        # Use device from config or fallback to detected device
        if 'hardware' in self.config and 'device' in self.config['hardware']:
            self.device = torch.device(self.config['hardware']['device'])
        else:
            # Fallback to detected device
    ...
    def _setup_environment(self):
        ...
        if torch.cuda.is_available():
            ...
            # Set device for all torch operations
    ...
    def _setup_monitoring(self):
        ...
        try:
            import wandb
            wandb_project = self.config.get('logging', {}).get('wandb_project', 'hms-brain-activity-production')
            wandb.init(
        ...
        try:
            import mlflow
            mlflow_enabled = self.config.get('logging', {}).get('mlflow_enabled', False)
            mlflow_tracking_uri = self.config.get('logging', {}).get('mlflow_tracking_uri')
            
            if mlflow_enabled and mlflow_tracking_uri and mlflow_tracking_uri != "null":
                mlflow.set_tracking_uri(mlflow_tracking_uri)
                mlflow.set_experiment("HMS-Novita-Enhanced")
                logger.info("✅ MLflow monitoring initialized")
            else:
                logger.info("MLflow disabled in configuration, skipping MLflow")
        except ImportError:
    ...
    def stage_preprocessing(self) -> bool:
        ...
        try:
            # Initialize preprocessors with GPU acceleration
            eeg_preprocessor = EEGPreprocessor(self.config, use_adaptive=False, device=str(self.device))
            spectrogram_generator = SpectrogramGenerator(self.config, device=str(self.device))
            
            # Load raw data
            raw_data_path = Path(self.config['dataset']['raw_data_path'])
            train_df = pd.read_csv(raw_data_path / "train.csv")
            
            logger.info(f"Processing {len(train_df)} EEG samples on {self.device}...")
            ...
            for batch_idx in tqdm(range(num_batches), desc="Processing batches"):
                ...
                for _, row in batch_df.iterrows():
                    ...
                    eeg_file = raw_data_path / "train_eegs" / f"{row['eeg_id']}.parquet"
                    if eeg_file.exists():
                        eeg_data = pd.read_parquet(eeg_file)
                    else:
                        # Fallback to CSV if parquet doesn't exist
                        eeg_file = raw_data_path / "train_eegs" / f"{row['eeg_id']}.csv"
                        eeg_data = pd.read_csv(eeg_file)
                    
                    # Get actual channel names from data
                    actual_channels = list(eeg_data.columns)
                    
                    # Align channels with expected channels
                    aligned_channels = []
                    aligned_data = []
                    
                    for expected_ch in channel_names:
                        if expected_ch in actual_channels:
                            aligned_channels.append(expected_ch)
                            aligned_data.append(eeg_data[expected_ch].values)
                        else:
                            # If channel is missing, use zeros
                            aligned_channels.append(expected_ch)
                            aligned_data.append(np.zeros(len(eeg_data)))
                    
                    # Convert to numpy array with correct shape (channels x samples)
                    eeg_array = np.array(aligned_data)
                    
                    # Verify shape
                    if eeg_array.shape[0] != len(channel_names):
                        logger.warning(f"Channel alignment failed for {eeg_file}: {eeg_array.shape[0]} channels, expected {len(channel_names)}")
                        continue
                    
                    eeg_batch.append(eeg_array)
                
                # If no valid EEGs in batch, skip
                if len(eeg_batch) == 0:
                    logger.warning(f"No valid EEG files in batch {batch_idx}, skipping batch.")
                    continue
                
                # Normalize EEG lengths to ensure consistent shapes
                target_length = self.config['eeg'].get('target_length', 10000)  # Default 10 seconds at 1000Hz
                normalized_eeg_batch = []
                
                for i, eeg_array in enumerate(eeg_batch):
                    try:
                        current_length = eeg_array.shape[1]
                        if current_length != target_length:
                            # Resize EEG to target length
                            if current_length > target_length:
                                # Truncate to target length
                                normalized_eeg = eeg_array[:, :target_length]
                            else:
                                # Pad with zeros to target length
                                normalized_eeg = np.zeros((eeg_array.shape[0], target_length), dtype=eeg_array.dtype)
                                normalized_eeg[:, :current_length] = eeg_array
                            
                            logger.debug(f"Normalized EEG {i} from length {current_length} to {target_length}")
                        else:
                            normalized_eeg = eeg_array
                        
                        normalized_eeg_batch.append(normalized_eeg)
                    except Exception as e:
                        logger.warning(f"Failed to normalize EEG length for sample {i}: {e}")
                        # Use zeros as fallback
                        normalized_eeg_batch.append(np.zeros((len(channel_names), target_length), dtype=np.float32))
                
                # Process EEG signals sequentially to avoid multiprocessing issues
                batch_eeg = []
                for eeg_data in normalized_eeg_batch:
                    try:
                        preprocessed_eeg, _ = eeg_preprocessor.preprocess_eeg(eeg_data, channel_names)
                        batch_eeg.append((preprocessed_eeg, {}))
                    except Exception as e:
                        logger.warning(f"Failed to preprocess EEG sample: {e}")
                        # Use original data as fallback
                        batch_eeg.append((eeg_data, {}))
                
                all_eeg_data.extend([eeg for eeg, _ in batch_eeg])
                
                # Generate spectrograms
                batch_spectrograms = spectrogram_generator.generate_batch([eeg for eeg, _ in batch_eeg])
                
                # Ensure all spectrograms have consistent shapes
                if batch_spectrograms:
                    # Get the shape of the first valid spectrogram
                    target_shape = batch_spectrograms[0].shape
                    
                    # Pad or truncate all spectrograms to match the target shape
                    normalized_spectrograms = []
                    for spec in batch_spectrograms:
                        if spec.shape != target_shape:
                            # Create a new array with target shape
                            normalized_spec = np.zeros(target_shape, dtype=spec.dtype)
                            # Copy as much as possible
                            slices = tuple(slice(0, min(s1, s2)) for s1, s2 in zip(spec.shape, target_shape))
                            normalized_spec[slices] = spec[slices]
                            normalized_spectrograms.append(normalized_spec)
                        else:
                            normalized_spectrograms.append(spec)
                    
                    batch_spectrograms = normalized_spectrograms
            ...
            if all_eeg_data:
                # Get the shape of the first valid EEG data
                eeg_target_shape = all_eeg_data[0].shape
                logger.info(f"Target EEG shape: {eeg_target_shape}")
                normalized_eeg_data = []
                
                for i, eeg in enumerate(all_eeg_data):
                    try:
                        if eeg.shape != eeg_target_shape:
                            logger.warning(f"EEG sample {i} shape {eeg.shape} doesn't match target {eeg_target_shape}, normalizing")
                            # Create a new array with target shape
                            normalized_eeg = np.zeros(eeg_target_shape, dtype=eeg.dtype)
                            # Copy as much as possible
                            slices = tuple(slice(0, min(s1, s2)) for s1, s2 in zip(eeg.shape, eeg_target_shape))
                            normalized_eeg[slices] = eeg[slices]
                            normalized_eeg_data.append(normalized_eeg)
                        else:
                            normalized_eeg_data.append(eeg)
                    except Exception as e:
                        logger.warning(f"Failed to normalize EEG sample {i}: {e}, using zeros")
                        normalized_eeg_data.append(np.zeros(eeg_target_shape, dtype=np.float32))
                
                all_eeg_data = normalized_eeg_data
                
                # Verify all EEG shapes are now consistent
                eeg_shapes = [eeg.shape for eeg in all_eeg_data]
                unique_shapes = set(eeg_shapes)
                if len(unique_shapes) > 1:
                    logger.error(f"EEG shapes still inconsistent after normalization: {unique_shapes}")
                    # Force all to the most common shape
                    most_common_shape = max(unique_shapes, key=eeg_shapes.count)
                    logger.info(f"Forcing all EEG data to shape: {most_common_shape}")
                    forced_eeg_data = []
                    for i, eeg in enumerate(all_eeg_data):
                        if eeg.shape != most_common_shape:
                            forced_eeg = np.zeros(most_common_shape, dtype=eeg.dtype)
                            slices = tuple(slice(0, min(s1, s2)) for s1, s2 in zip(eeg.shape, most_common_shape))
                            forced_eeg[slices] = eeg[slices]
                            forced_eeg_data.append(forced_eeg)
                        else:
                            forced_eeg_data.append(eeg)
                    all_eeg_data = forced_eeg_data
            
            if all_spectrograms:
                # Get the shape of the first valid spectrogram
                spec_target_shape = all_spectrograms[0].shape
                logger.info(f"Target spectrogram shape: {spec_target_shape}")
                normalized_spectrograms = []
                
                for i, spec in enumerate(all_spectrograms):
                    try:
                        if spec.shape != spec_target_shape:
                            logger.warning(f"Spectrogram {i} shape {spec.shape} doesn't match target {spec_target_shape}, normalizing")
                            # Create a new array with target shape
                            normalized_spec = np.zeros(spec_target_shape, dtype=spec.dtype)
                            # Copy as much as possible
                            slices = tuple(slice(0, min(s1, s2)) for s1, s2 in zip(spec.shape, spec_target_shape))
                            normalized_spec[slices] = spec[slices]
                            normalized_spectrograms.append(normalized_spec)
                        else:
                            normalized_spectrograms.append(spec)
                    except Exception as e:
                        logger.warning(f"Failed to normalize spectrogram {i}: {e}, using zeros")
                        normalized_spectrograms.append(np.zeros(spec_target_shape, dtype=np.uint8))
                
                all_spectrograms = normalized_spectrograms
                
                # Verify all spectrogram shapes are now consistent
                spec_shapes = [spec.shape for spec in all_spectrograms]
                unique_shapes = set(spec_shapes)
                if len(unique_shapes) > 1:
                    logger.error(f"Spectrogram shapes still inconsistent after normalization: {unique_shapes}")
                    # Force all to the most common shape
                    most_common_shape = max(unique_shapes, key=spec_shapes.count)
                    logger.info(f"Forcing all spectrograms to shape: {most_common_shape}")
                    forced_spec_data = []
                    for i, spec in enumerate(all_spectrograms):
                        if spec.shape != most_common_shape:
                            forced_spec = np.zeros(most_common_shape, dtype=spec.dtype)
                            slices = tuple(slice(0, min(s1, s2)) for s1, s2 in zip(spec.shape, most_common_shape))
                            forced_spec[slices] = spec[slices]
                            forced_spec_data.append(forced_spec)
                        else:
                            forced_spec_data.append(spec)
                    all_spectrograms = forced_spec_data
            
            # Final verification before numpy conversion
            if all_eeg_data:
                eeg_shapes = [eeg.shape for eeg in all_eeg_data]
                if len(set(eeg_shapes)) > 1:
                    logger.error(f"Final EEG shape check failed: {set(eeg_shapes)}")
                    logger.error(f"First 5 EEG shapes: {eeg_shapes[:5]}")
                    return False
                else:
                    logger.info(f"All EEG data have consistent shape: {eeg_shapes[0]}")
                
            if all_spectrograms:
                spec_shapes = [spec.shape for spec in all_spectrograms]
                if len(set(spec_shapes)) > 1:
                    logger.error(f"Final spectrogram shape check failed: {set(spec_shapes)}")
                    logger.error(f"First 5 spectrogram shapes: {spec_shapes[:5]}")
                    return False
                else:
                    logger.info(f"All spectrograms have consistent shape: {spec_shapes[0]}")
            
            # Convert to numpy arrays with consistent shapes
            try:
                logger.info(f"Converting {len(all_eeg_data)} EEG samples to numpy array...")
                eeg_array = np.array(all_eeg_data)
                logger.info(f"EEG array shape: {eeg_array.shape}")
                
                logger.info(f"Converting {len(all_spectrograms)} spectrograms to numpy array...")
                spectrogram_array = np.array(all_spectrograms)
                logger.info(f"Spectrogram array shape: {spectrogram_array.shape}")
                
                logger.info(f"Converting {len(all_labels)} labels to numpy array...")
                labels_array = np.array(all_labels)
                logger.info(f"Labels array shape: {labels_array.shape}")
                
            except Exception as e:
                logger.error(f"Failed to convert to numpy arrays: {e}")
                logger.error(f"EEG data types: {[type(eeg) for eeg in all_eeg_data[:5]]}")
                logger.error(f"EEG data shapes: {[eeg.shape if hasattr(eeg, 'shape') else 'no shape' for eeg in all_eeg_data[:5]]}")
                logger.error(f"Spectrogram types: {[type(spec) for spec in all_spectrograms[:5]]}")
                logger.error(f"Spectrogram shapes: {[spec.shape if hasattr(spec, 'shape') else 'no shape' for spec in all_spectrograms[:5]]}")
                return False
        ...
        try:
            ...
            for model_name in models_to_train:
                ...
                if model_name == 'resnet1d_gru':
                    model = ResNet1D_GRU(self.config)
    ...
    def stage_ensemble_training(self) -> bool:
        ...
        try:
            ...
            if self.training_state.models_trained.get('resnet1d_gru', False):
                resnet_model = ResNet1D_GRU(self.config)
