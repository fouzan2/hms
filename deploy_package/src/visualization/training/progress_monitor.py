...
class TrainingProgressMonitor:
    ...
    def update_metrics(self, epoch: int, metrics: Dict[str, float]):
        """
        Update metrics (for compatibility with tests).
        ...
        # Calculate iteration based on current metrics length
        iteration = len(self.metrics['iteration'])
        
        # Call the existing log_metrics method
        self.log_metrics(
            iteration=iteration,
            epoch=epoch,
    ...
    def plot_training_curves(self, training_history: Dict[str, List[float]]):
        """
        Plot training curves from history (for compatibility with tests).
        
        Args:
            training_history: Dictionary with training metrics history
        """
        # Convert training history to internal metrics format
        for key, values in training_history.items():
            if key in self.metrics:
                self.metrics[key] = values
        
        # Generate iterations if not provided
        if len(self.metrics['iteration']) == 0:
            self.metrics['iteration'] = list(range(len(self.metrics['train_loss'])))
        
        # Generate epochs if not provided
        if len(self.metrics['epoch']) == 0:
            self.metrics['epoch'] = list(range(len(self.metrics['train_loss'])))
        
        # Generate timestamps if not provided
        if len(self.metrics['timestamp']) == 0:
            base_time = datetime.now()
            self.metrics['timestamp'] = [base_time + timedelta(seconds=i) for i in range(len(self.metrics['train_loss']))]
